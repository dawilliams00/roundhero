          ...
          {skills.map(({ skill, ability, bonus, proficient, expertise }) => (
            <div key={skill} onClick={() => roll(skill, bonus)} title="Click to roll d20 + bonus" style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'6px 10px',background:'var(--bg-primary)',borderRadius:'var(--radius-sm)',cursor:'pointer'}}>
              <div style={{display:'flex',alignItems:'center',gap:6,minWidth:0}}>
                <div style={{width:8,height:8,borderRadius:'50%',flexShrink:0,background: expertise ? 'var(--warning)' : proficient ? 'var(--accent)' : 'var(--border)',border: proficient||expertise ? 'none' : '1px solid var(--border)'}}/>
                <span style={{color:'var(--text-primary)',fontSize:13,whiteSpace:'nowrap'}}>{skill}</span>
                <span style={{color:'var(--text-dim)',fontSize:11}}>({ability})</span>
              </div>
              <span style={{color:'var(--accent-light)',fontWeight:600,flexShrink:0,marginLeft:6}}>{bonus >= 0 ? `+${bonus}` : bonus}</span>
            </div>
          ))}
        </div>
